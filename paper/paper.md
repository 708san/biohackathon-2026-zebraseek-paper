...
abstract: |
  Rare disease differentials must reconcile evidence unevenly represented across clinical descriptions, facial phenotypes and disease resources. ZebraSeek integrates specialist phenotype and facial matching, semantic retrieval and large language model predictions into a traceable ranked differential with external-information checks. In 74 literature-derived cases spanning 19 diseases, ZebraSeek retrieved the recorded diagnosis at rank one in 50 cases (67.6%) and within five ranks in 59 (79.7%); 11 diagnoses were recovered beyond PubCaseFinder's top-five coverage, while five were lost during integration. The fixed top-five input limit prevents later reasoning from recovering diagnoses ranked more deeply by specialist tools. During DBCLS BioHackathon 2026, we expanded evaluation to 368 patients, 462 images and 54 disorders to measure this loss and compare simple strategies for choosing how many candidates to take from each tool. Structured outputs were also strengthened to preserve candidate identity and source URLs. End-to-end gains from case-specific candidate selection remain to be established.
---
...
This modular design also creates a practical limitation. Different types of patient information are processed by different specialist tools, and their internal scores are not assumed to be directly comparable. Downstream integration therefore receives ranked disease candidates and tool-specific metadata rather than one shared score for facial and HPO evidence. In the current workflow each component contributes only its top five candidates. A correct diagnosis ranked more deeply is consequently invisible to later LLM reasoning and verification, regardless of the quality of those downstream stages.

During DBCLS BioHackathon 2026, we focused on this fixed top-five limitation. We examined how many correct diagnoses are missed when only the first five candidates are passed downstream, how coverage changes when deeper rankings are considered, and whether case-specific strategies can select enough candidates without forwarding every deep-ranked disease to later verification. We also strengthened structured outputs so candidate identity and source information, including URLs where available, are preserved more explicitly through the integration pipeline.

Here we retain the original 74-case ZebraSeek results and component analyses, and add an expanded analysis of how many candidates to take from each tool using Phenopacket Store v0.1.27 linked to facial images. The original analysis evaluates complementary retrieval and candidate retention. The hackathon analysis measures the fixed top-five limitation and provides fixed-depth, zero-shot LLM and best-case retrospective references for subsequent case-specific candidate selection. Together these analyses preserve ZebraSeek's original aim: effective multimodal integration with a compact, reviewable and traceable differential.
...
## Basic candidate-selection strategies show the trade-off between coverage and candidate count

Simply forwarding every deep-ranked candidate to the existing verification pipeline is not an attractive solution. At rank 30, PubCaseFinder and GestaltMatcher together can contribute almost 60 unique diseases per image-level instance, each of which may require normalization, evidence retrieval and LLM-based comparison. The practical problem is therefore to take enough candidates from each tool to preserve useful diagnoses without sending every deep-ranked disease to later processing.

PubCaseFinder operates on HPO information and GestaltMatcher on facial images, and their scores have different meanings and scales. We therefore do not assume that one threshold can be applied directly to both tools. Instead, we compare simple strategies for deciding how many candidates to take from each ranking.

Fixed candidate depths provide reference points (Table 1). PCF@30 reached 83.55% coverage with 30 candidates on average. The combined PCF@30 and GM@30 lists reached 88.96% coverage but produced 57.57 unique candidates on average after duplicate diseases were removed. Two preliminary zero-shot LLM strategies, using no task-specific training examples, selected how many candidates to take from each tool for each input. The input-only strategy achieved 84.38% coverage with 19.28 candidates on average, whereas the strategy that also received tool results achieved 84.60% with 18.18 candidates on average. These LLM-based conditions include 461 image-level instances and are treated as basic feasibility baselines rather than evidence that LLM-based selection is optimal.

As a best-case retrospective reference, we used the known diagnosis to identify the smallest number of candidates that would still include the correct diagnosis whenever it was available within the evaluated rankings. This reference reaches the same 88.96% coverage as taking the top 30 candidates from both PCF and GM while retaining 2.66 candidates on average. Because it uses the known diagnosis, it is not an executable diagnostic method or a performance target. It only shows how much the candidate count could theoretically be reduced if the appropriate depth were known in advance.
...
The hackathon work addresses a different but adjacent failure mode: useful diagnoses that never enter the integration stage. The current workflow accepts only the top five candidates from each specialist component. Because ZebraSeek combines different input types through specialist tools whose scores are not directly comparable, the system must decide how many ranked disease candidates to take from each tool. The expanded analysis makes this problem visible and measurable.

The expanded v0.1.27 analysis shows that deeper PCF and GM rankings contain additional correct diagnoses beyond the fixed top-five lists, while taking every deep-ranked candidate greatly increases the number of diseases passed to later stages. These observations define a practical tension for ZebraSeek: a shallow candidate list limits what later reasoning can recover, whereas a very deep list increases the amount of evidence retrieval and LLM comparison required. How many candidates to take from each tool is therefore part of the system design rather than a neutral preprocessing choice.

The comparisons among fixed candidate depths, zero-shot LLM choices and the best-case retrospective reference provide an initial validation of this framing. They show that coverage and candidate count can vary substantially depending on how many candidates are taken from each tool, but they do not establish a final case-specific selection method. The best-case reference uses the known diagnosis and only estimates the potential reduction in candidate count. The current zero-shot LLM strategies are baselines. Future approaches can be compared with fixed-depth combinations, simple rules based on within-tool rank or score patterns, feature-based selection methods, and sequential rules that decide whether more candidates should be taken, using separate development and test sets.

This work on candidate-list depth complements rather than replaces the original ZebraSeek contribution. The original benchmark demonstrates complementary retrieval, candidate retention and a higher observed final recall within its evaluation setting. The hackathon analysis identifies an upstream source of avoidable candidate loss and establishes measurements for improving it. The structured-output revision simultaneously strengthens source traceability by preserving candidate identifiers and source URLs through downstream processing. Together, these changes move ZebraSeek toward a workflow that is effective in candidate integration, economical in what it chooses to verify and explicit about where its evidence came from.

These aims should be distinguished from existing multimodal models. PhenoScore and GestaltMML already establish important precedents for integrating facial and clinical representations [@PhenoScore2023; @GestaltMML2026]. ZebraSeek instead combines proposals from specialist tools with LLM-based integration and external-information checks. Its proposed value is not a new facial representation or the first multimodal approach, but a modular workflow in which candidate selection, candidate retention, downstream verification and source traceability can be examined separately.

Several limitations constrain the present interpretation. The original benchmark is retrospective, small and selected for availability of both phenopackets and facial images. The expanded analysis contains 462 images from 368 patients, so image-level observations are not independent patient-level tests. The reported 54 disorders remain unevenly represented, and the zero-shot LLM analysis contains one fewer instance than the fixed-depth analyses. No controlled end-to-end comparison has yet shown that case-specific candidate selection improves final ZebraSeek Recall@5, latency, token use or monetary cost. Repeated-run variability, disease-stratified performance and patient-wise held-out evaluation are also required.
...
The resulting research question is broader than whether adding modalities improves accuracy: how can complementary specialist evidence be explored deeply enough to avoid preventable candidate loss, integrated selectively enough to remain practical, and presented with enough source information to be independently reviewed? The current ZebraSeek implementation, original 74-case benchmark and hackathon analysis of candidate-list depth provide an initial framework for answering these questions rather than a completed clinical validation.
...
The zero-shot LLM conditions, which used no task-specific training examples, asked an LLM to choose candidate depths per input. One condition used input information only; a second also received component-tool results. Aggregate coverage and mean candidate counts are reported for 461 image-level instances in these conditions.
