...
The relevant evidence is distributed across different forms of patient information and biomedical knowledge. The Human Phenotype Ontology (HPO) provides a structured vocabulary for clinical abnormalities [@HPO2024]. PubCaseFinder uses HPO terms to retrieve phenotype-similar case reports and prioritize diseases [@PubCaseFinder2018]. GestaltMatcher instead converts patient portraits into a facial-phenotype representation and searches for similar cases in a learned phenotype space [@GestaltMatcher2022]. These tools view the patient from different perspectives, so a disease weakly represented in one modality can remain recognizable in another.

Facial images are particularly useful as a complementary phenotype source because they can be processed directly by a specialist image model without requiring every visible feature to be manually recognized, described and encoded as HPO terms. GestaltMatcher showed that patients with the same molecular diagnosis can be matched through facial phenotype similarity even when the disorder was not represented in the model's training set [@GestaltMatcher2022]. PEDIA likewise reported that information from frontal photographs extends beyond what clinical terms alone capture [@PEDIA2019]. We therefore treat facial imaging not as an objective ground truth, but as a quantitative, machine-readable phenotype representation that can complement manually annotated clinical features. Its performance can still depend on image quality, age, population representation and the coverage of the reference data.

Previous multimodal studies have already demonstrated the value of combining such information. PhenoScore combines facial analysis with HPO-based phenotypic similarity [@PhenoScore2023]. GestaltMML integrates facial images with demographic and clinical information [@GestaltMML2026]. PEDIA combines image-derived, phenotype-similarity and molecular scores and uses a linear support vector machine to produce a unified gene-prioritization score [@PEDIA2019]. SHEPHERD uses phenotype-aware knowledge-graph representations for several rare-disease diagnostic tasks [@SHEPHERD2025]. These studies establish that multimodal evidence can improve rare-disease analysis, but they also expose a systems question beyond whether fusion is useful: how should outputs from independently developed specialist tools be selected and carried forward when their representations and score scales differ?

Large language models (LLMs) provide a flexible integration layer for this problem because they can reason over candidate identities, ranks, tool-specific metadata, patient findings and retrieved external evidence rather than requiring all information to be reduced to a single predefined numeric score. This flexibility should not be interpreted as evidence that LLM integration is inherently more accurate than machine-learning score fusion; it introduces its own dependence on model reasoning, normalization and prompt design. Recent LLM-based systems include DeepRare [@DeepRare2026], RareAgents [@RareAgents2026] and MEDDxAgent [@MEDDxAgent2025]. DeepRare is particularly relevant because it orchestrates more than 40 specialist tools and knowledge sources and returns evidence-linked, traceable reasoning [@DeepRare2026]. In its published input formulation, however, DeepRare accepts free-text phenotypes, structured HPO terms and genetic testing results or VCF data rather than facial photographs [@DeepRare2026].

ZebraSeek focuses on the candidate-level problem created when these different modalities are handled by separate specialist tools. Clinical phenotypes, facial images and related information are first converted into ranked disease candidates. The candidate records are then integrated by an LLM, checked against external medical information and reduced to a final ranked differential. Facial evidence is introduced through GestaltMatcher rather than by asking the LLM to interpret a patient photograph directly. This approach preserves the specialist model for each modality while exposing a common disease-candidate interface to the integration stage. It also allows candidate identity and source information to be retained through later processing.

The distinction is important for extensibility. PEDIA demonstrates effective integration of predefined numeric features through a trained linear classifier [@PEDIA2019]. ZebraSeek instead aims to support heterogeneous candidate records whose useful context can differ by tool and by patient, including within-tool rank or score, observed and excluded findings, and externally retrieved evidence. The intended advantage is therefore not a claim of superior predictive performance, but a more flexible integration interface for heterogeneous specialist outputs and a clearer separation between candidate generation, candidate selection and downstream verification.

The current implementation also exposes clear limitations. Each component contributes its top five candidates using a fixed heuristic. A disease ranked below five is invisible to all later LLM reasoning and verification, even if the downstream reasoning is strong. Conversely, simply increasing every list to dozens of candidates increases the number of diseases that must be normalized, searched, compared and verified by the LLM. The specialist scores are not directly interchangeable: PubCaseFinder ranks diseases from HPO information, whereas GestaltMatcher ranks them from facial images. Thus, the practical problem is not only how to combine different modalities, but also how many candidates from each modality-specific tool should be allowed to reach the expensive downstream stages.
...
Facial phenotyping provides a concrete example of why this modularity matters. A facial photograph can be processed directly by a specialist image model rather than requiring every dysmorphic feature to be manually translated into HPO terms. GestaltMatcher can place portraits in a phenotype space that supports matching of ultra-rare disorders, including disorders absent from its training set [@GestaltMatcher2022]. PEDIA found that image-derived facial information contributed information beyond clinical terms and materially improved gene prioritization when combined with phenotype and molecular evidence [@PEDIA2019]. These results support facial images as a complementary phenotype source, although they do not make image analysis bias-free or universally reliable.

The original 74-case benchmark primarily supports the effectiveness of combining such complementary candidate sources. ZebraSeek achieved higher observed top-k recall than each individual component within that evaluation setting. More importantly, the case-level pattern explains how that gain arose: 11 diagnoses were recovered beyond PubCaseFinder's top-five coverage, whereas five diagnoses retrieved by PubCaseFinder were lost during integration. This shows both the value and the risk of candidate-level integration. Expanding the candidate pool can recover diagnoses missed by an individual tool, but the downstream ranking process can still discard useful source-specific candidates. The seven successful cases in which GestaltMatcher alone contained the recorded diagnosis within its top five are consistent with facial evidence being complementary, but they are not a controlled facial-image ablation and should not be interpreted causally.

Previous multimodal systems provide important foundations for this design. PhenoScore integrates facial analysis with HPO-based phenotypic similarity [@PhenoScore2023]. GestaltMML combines facial images with demographic and clinical information [@GestaltMML2026]. PEDIA combines image, phenotype and molecular scores using a linear support vector machine [@PEDIA2019]. These approaches demonstrate that fixed machine-learning fusion of predefined features can be highly effective. ZebraSeek explores a different integration interface: candidate records from specialist tools remain identifiable and can be reconsidered together with tool-specific ranks, patient findings, negative findings and retrieved medical evidence. This provides greater flexibility in what information can be considered at integration time, but it also increases dependence on LLM reasoning and therefore requires explicit controls for candidate identity, reproducibility and verification.

DeepRare extends LLM-based rare-disease diagnosis further by orchestrating many tools and knowledge sources and returning reasoning linked to verifiable evidence [@DeepRare2026]. Its published input formulation includes free-text clinical descriptions, HPO terms and genetic testing results or VCF data, but not facial photographs [@DeepRare2026]. ZebraSeek therefore should not be positioned as the first agentic or traceable system. Its narrower contribution is to make modality-specific specialist outputs explicit at the candidate level and to treat the number of candidates passed from each specialist tool into downstream LLM reasoning as a design variable.
...
The same problem is likely to become more important as additional imaging modalities are added. Eye2Gene, for example, uses fundus autofluorescence, infrared imaging and optical coherence tomography to generate gene-level predictions for inherited retinal diseases [@Eye2Gene2025]. In a subset of 130 individuals, its image-based gene rank was higher than or equal to an HPO-based Exomiser-hiPHIVE rank in more than 75% of patients [@Eye2Gene2025]. Such organ-specific image models illustrate a path beyond facial phenotyping: a future ZebraSeek-like architecture could receive candidate outputs from retinal imaging, facial imaging, phenotype search and other specialist modalities, map them to a common disease or gene representation, and then decide which candidates warrant deeper verification. Facial images are therefore best viewed as an initial image modality rather than the endpoint of the architecture.

This extension also sharpens the efficiency question. As more modalities are introduced, forwarding every candidate from every tool to the LLM becomes less attractive. A scalable system will need to determine not only how to combine heterogeneous evidence, but also which modality-specific outputs are informative enough for a given patient to justify additional retrieval and reasoning. The BioHackathon candidate-depth analysis is an initial step toward that broader goal because it isolates one tractable version of the problem: how far to explore ranked outputs from HPO-based and facial-image-based specialist tools before downstream integration.
