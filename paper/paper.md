...
Previous multimodal studies have already demonstrated the benefit of combining such information. PhenoScore [@PhenoScore2023] combines facial analysis with HPO-based phenotypic similarity. GestaltMML [@GestaltMML2026] integrates facial images with demographic and clinical information. PEDIA [@PEDIA2019] combines image-derived, phenotype-similarity and molecular scores and uses a linear support vector machine to produce a unified gene-prioritization score. SHEPHERD [@SHEPHERD2025] uses phenotype-aware knowledge-graph representations for several rare-disease diagnostic tasks. These studies establish that multimodal fusion can be effective; the remaining systems question is how to make such integration scalable when the available specialist models operate on different modalities, return different candidate spaces and use scores that are not calibrated to one another.

PEDIA provides a useful example of both the value and the limits of fixed score fusion. Its linear support vector machine learns a single decision rule over predefined per-gene scores [@PEDIA2019]. This is an effective and interpretable way to combine the selected inputs, but the fusion rule itself does not dynamically decide, for a particular patient, which specialist modality should be explored more deeply or how much downstream attention a modality-specific candidate should receive. As the number of specialist image and phenotype tools grows, the problem is therefore not only how to combine scores, but how to select and preserve the most informative candidate outputs before detailed verification.
...
Existing multimodal methods demonstrate that combining modalities can be highly effective. PhenoScore [@PhenoScore2023] integrates facial and HPO-based information, and GestaltMML [@GestaltMML2026] combines facial images with demographic and clinical information. PEDIA [@PEDIA2019] uses a linear support vector machine to combine predefined image, phenotype and molecular scores. That fixed linear rule is a strong baseline for multimodal fusion, but it is conceptually different from deciding, for each patient, which specialist outputs should be explored, retained and verified. ZebraSeek explores the latter interface: candidate records remain identifiable and can be reconsidered together with within-tool ranks or scores, observed and excluded findings and retrieved medical evidence. This is a flexibility claim, not a demonstrated accuracy advantage over trained fusion models.
